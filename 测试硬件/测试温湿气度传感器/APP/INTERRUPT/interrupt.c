...
void USART2_IRQHandler(void) {
	...
	if((USART_GetITStatus(USART2, USART_IT_IDLE)) != RESET) {
		...
		//获取本次串口2接收数据大小
		UART2_RxCounter = UART2_RXBUFF_SIZE - DMA_GetCurrDataCounter(DMA1_Channel6);
		
		//重新设置接收的长度
		DMA_Cmd(DMA1_Channel6, DISABLE); //先禁止DMA1通道6
		DMA1_Channel6->CNDTR = UART2_RXBUFF_SIZE; //重新设置接收的数据长度
