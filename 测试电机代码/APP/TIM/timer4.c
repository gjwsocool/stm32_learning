#include "timer4.h"
#include "led.h"

//定义定时器4初始化函数
void TIMER4_Init(void) {
	TIM_TimeBaseInitTypeDef TIMER4_Config;
	NVIC_InitTypeDef	NVIC_Config;
	
	//启动定时器TIMER4时钟
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE);
	
	//初始化定时器4TIMER4的工作参数
	TIMER4_Config.TIM_Period = ARR;		//初始化自动装载值
	TIMER4_Config.TIM_Prescaler = PSC;	//初始化分频系数
	TIMER4_Config.TIM_ClockDivision = TIM_CKD_DIV1;	//1分频,72MHz
	TIMER4_Config.TIM_CounterMode = TIM_CounterMode_Up;	//向上计数模式
	TIM_TimeBaseInit(TIM4, &TIMER4_Config);	//初始化定时器4
	
	//先清除定时器中断到来的标志位，否则上电默认产生一次定时器中断信号
	TIM_ClearITPendingBit(TIM4, TIM_IT_Update);
	//使能TIMER4的溢出中断功能
	TIM_ITConfig(TIM4, TIM_IT_Update, ENABLE);
	
	//初始化NVIC的定时器4通道的中断功能
	NVIC_Config.NVIC_IRQChannel = TIM4_IRQn;	//指定定时器4通道中断功能
	NVIC_Config.NVIC_IRQChannelPreemptionPriority = 2;		//指定抢占优先级
	NVIC_Config.NVIC_IRQChannelSubPriority = 3; //指定子优先级
	NVIC_Config.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Init(&NVIC_Config);
}

//定义定时器4的中断处理函数
void TIM4_IRQHandler(void) {
	//定时器溢出产生中断，首先判断是否是溢出中断
	if(TIM_GetITStatus(TIM4, TIM_IT_Update)) {
		LED1 = !LED1;
	}
	//清除溢出中断到来标志位
	TIM_ClearITPendingBit(TIM4, TIM_IT_Update);
}

//定义定时器4启动函数
void TIMER4_Start(void) {
	TIM_Cmd(TIM4, ENABLE);
}

//定义定时器4关闭函数
void TIMER4_Stop(void) {
	TIM_Cmd(TIM4, DISABLE);
}


